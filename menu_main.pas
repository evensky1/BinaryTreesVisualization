...
var
ensurance: integer;
begin  //”бедитс€, точно ли пользователь желает выйти
 ensurance := messagedlg('¬ы уверены, что хотите выйти? ', mtConfirmation, mbYesNo, 0);
 if ensurance = 6 then
 main_menu.close;
end;
...
begin
 binary_form.show;            //вывод формы дл€ визуализаций
 main_menu.enabled := FALSE; //форма главного меню становитс€ недоступной
end;

procedure Tmain_menu.btnDirClick(Sender: TObject);
begin //событие нажати€, дл€ перехода к созданию дерева сразу через его загрузку из файла
